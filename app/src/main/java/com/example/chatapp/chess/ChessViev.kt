...
class ChessView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    // Добавляем свойство для переворота доски
    var isFlipped: Boolean = false
        set(value) {
            field = value
            invalidate() // Перерисовываем при изменении
        }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                val touchCol = ((event.x - originX) / cellSide).toInt()
                val touchRow = ((event.y - originY) / cellSide).toInt()

                // Применяем переворот при обработке касания
                fromCol = if (isFlipped) 7 - touchCol else touchCol
                fromRow = if (isFlipped) 7 - touchRow else touchRow

                // Проверка, что фигура принадлежит текущему игроку
                chessDelegate?.pieceAt(Square(fromCol, fromRow))?.let {
                    movingPiece = it
                    movingPieceBitmap = bitmaps[it.imageResId]
                }
            }
            ...
            MotionEvent.ACTION_UP -> {
                ...
                // Применяем переворот при обработке касания
                val toCol = if (isFlipped) 7 - touchCol else touchCol
                val toRow = if (isFlipped) 7 - touchRow else touchRow
            ...
            for (col in 0 until 8)
                chessDelegate?.pieceAt(Square(col, row))?.let { piece ->
                    if (piece != movingPiece) {
                        // Применяем переворот при отрисовке
                        val displayCol = if (isFlipped) 7 - col else col
                        val displayRow = if (isFlipped) 7 - row else row
                        drawPieceAt(canvas, displayCol, displayRow, piece.imageResId)
                    }
                }
    ...
    private fun drawPieceAt(canvas: Canvas, col: Int, row: Int, resID: Int) =
        canvas.drawBitmap(bitmaps[resID]!!, null, RectF(originX + col * cellSide,originY + row * cellSide,originX + (col + 1) * cellSide,originY + (row + 1) * cellSide), paint)
    ...
    private fun drawChessboard(canvas: Canvas) {
        for (row in 0 until 8)
            for (col in 0 until 8) {
                // Применяем переворот при отрисовке доски
                val displayCol = if (isFlipped) 7 - col else col
                val displayRow = if (isFlipped) 7 - row else row
                drawSquareAt(canvas, displayCol, displayRow, (col + row) % 2 == 1)
            }
    }
